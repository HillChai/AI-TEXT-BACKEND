...
async def create_question_api(question: QuestionCreate, db: AsyncSession = Depends(get_db)):
    """
    创建一个新的问题。
    """
...
async def get_questions_history(
    ...
    # 查询所有属于用户的记录
    all_questions = await get_questions_by_user(db, user_id)
...
async def list_questions_api(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
    """
    获取问题列表，支持分页。
    """
    return await crud_question.get_all_questions(db, skip, limit)


@router.put("/update-by-original-content", summary="通过原始内容更新问题")
async def update_by_original_content(request: dict, db: AsyncSession = Depends(get_db)):
    # 获取请求参数
...
async def update_question_api(question_id: int, question_update: QuestionUpdate, db: AsyncSession = Depends(get_db)):
    """
    更新问题内容。
    """
